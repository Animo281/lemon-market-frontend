...
export default function PhaseIndicator({ round, total, infoMode }: Props) {
  ...
  return (
    <div className={`inline-flex items-center gap-2.5 rounded-xl border px-4 py-2 text-sm transition-colors ${
      full
        ? 'bg-lime-500/10 border-lime-500/35 text-lime-400'
        : 'bg-coral-500/10 border-coral-500/35 text-coral-400'
    }`}>
      ...
      <span className="font-mono text-mkt-300">
        Runde <strong className="text-mkt-100">{round}</strong>
        <span className="text-mkt-500">/{total}</span>
      </span>

      <span className={`text-[10px] font-semibold uppercase tracking-[0.1em] px-2 py-0.5 rounded-md border ${
        full
