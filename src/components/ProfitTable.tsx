    ...
    <div className="space-y-6">
        ...
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr className="border-b border-mkt-800">
              ...
              {sortedSellers.map(s => (
                <th key={s.id} className="text-left py-3 pr-5 font-semibold text-lemon-400 text-[10px] uppercase tracking-wide whitespace-nowrap">
                  {s.name}
                </th>
              ))}
          ...
          <tbody>
            {results.map(r => (
              <tr key={r.round} className="border-b border-mkt-800/40 hover:bg-mkt-850/30 transition-colors">
                ...
                <td className="py-3 font-mono font-bold text-lemon-400">€{r.totalSurplus.toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            {sortedSellers.map(s => (
              <div key={s.id} className="flex justify-between items-center">
                <span className="text-lemon-400 text-sm font-semibold">{s.name}</span>
          ...
          <div className="space-y-2">
            {sortedBuyers.map(b => (
              <div key={b.id} className="flex justify-between items-center">
                <span className="text-ice-400 text-sm font-semibold">{b.name}</span>
                <span className="font-mono font-bold text-mkt-100 text-sm">
        ...
        <div className="panel p-5 flex flex-col items-center justify-center text-center">
          ...
          <div className="font-display font-bold text-lemon-400 leading-none"
